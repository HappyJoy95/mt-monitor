...
（后者需进入拣货完成前 6 分钟窗口才推送），把完整原始接口响应存档到 `raw/`，
并生成可读的订单摘要到 `data/`。
...
首次直接使用。`pull` 每轮抓取**两个互斥标签**的订单列表并合并：

| 标签 | 请求体 tag | 内容 |
|------|-----------|------|
| 待接单 | `order_new` | 待商家接单的订单 |
| 进行中 | `order_processing` | 待发起配送及之后所有在途状态 |

两类订单集合互不重叠（实测：一个月的抓取里从未有响应同时含两类），只抓一个必然
漏掉另一个——这正是「待发起配送抓不到」的根因。响应按请求体的 `tag` 精确匹配，
不靠先后顺序。再由 `normalize` 过滤出「待接单」与「待发起配送」两种状态并推送。
拉取/导入后，只要有订单就会
...
只记「成功 N 笔」无法回答"我的群收到没有"，所以每个推送目标都会单独记一行，
写进当轮日志（`logs/pull-*.log`，见上文 `pull-logged`）：

```
推送主群 key=31eae693… 订单 2602307902705035686（待接单）→ 成功
推送门店群 华为授权体验店（新业广场店） key=d82fed27… 订单 2602307902705035686（待接单）→ 成功
⚠️ 推送门店群 华为授权体验店（城阳万达广场店） key=bfb3328c… 订单 …（待接单）失败：…
ℹ️ 门店 '某未登记门店' 不在映射表（config/store_webhooks.json），跳过门店群推送（订单 …）。
ℹ️ 门店 华为授权体验店（新业广场店）当前非营业时间（09:30:00~21:30:00），跳过门店群推送（订单 …）。
```

- key 只记前 8 位（`_key_hint`），既不泄露密钥又能对上具体机器人。
- 以前"门店不在映射表"是**完全静默**跳过的，现在也会记一行，避免漏推无人知情。
- 客户端只在企业微信返回 `errcode == 0` 时才算成功，所以"成功"= 平台已接收；
  消息渲染到群聊属平台侧行为，日志能证明已接收、发到了哪个机器人。
...
- **每轮抓两个互斥标签**（`TARGET_TABS` = 「进行中」+「待接单」）：所有标签打的是
  同一个接口、URL 完全一样，只有请求体里的 `tag` 能区分（`进行中`=`order_processing`、
  `待接单`=`order_new`）。两类订单集合**互不重叠**（实测一个月的抓取里从未同现），
  只抓一个必然漏掉另一个；早期版本只接受"最后一个响应"，于是每分钟随机只覆盖一类，
  **待发起配送因此长期抓不到**。现在按 `tag` 精确匹配响应，并把两个标签的结果按订单号
  去重合并后归档（`data.capturedTabs` 记录来源标签，便于事后核对）。当页面已停在目标
  标签时，先点另一个标签再点回来，否则 SPA 不会重新请求。订单范围仍由 `normalize` 的
  `VALID_STATUSES` + 拣货时间窗口决定。
- **「全部」标签**（`tag=order_all`）按时间倒序返回含已取消/已收货的全部订单，可用于
  人工追溯某单（例如判断用户是否退单），但**不作为抓取源**：它按页返回
  （pageSize=10），订单量大时在途订单可能不在第一页，当数据源反而会漏推。
